...
#define MYDISK_DEVICE_SIZE 1024
...
{
	memcpy(dev_data + sector_off * MYDISK_SECTOR_SIZE, buffer,
		sectors * MYDISK_SECTOR_SIZE);
}
void ramdevice_read(sector_t sector_off, u8 *buffer, unsigned int sectors)
{
	memcpy(buffer, dev_data + sector_off * MYDISK_SECTOR_SIZE,
		sectors * MYDISK_SECTOR_SIZE);
}
